...
void testClient()
{
    ...
    corpc::PbRpcChannel channel(addrs, corpc::LoadBalanceCategory::Round);
    ...
    std::cout << "peer addr: " << rpcControllerName.PeerAddr()->toString() << std::endl;
    ...
    std::cout << "peer addr: " << rpcControllerAge.PeerAddr()->toString() << std::endl;
    ...
    if (ageRes.ret_code() != 0) {
        std::cout << "call query_age return bad result from corpc server, ret = " << ageRes.ret_code() << ", and res_info = " << ageRes.res_info();
        USER_LOG_DEBUG << "call query_age return bad result from corpc server, ret = " << ageRes.ret_code() << ", and res_info = " << ageRes.res_info();
        return;
    }

    std::cout << "Success get query_age response from corpc server, res = " << ageRes.ShortDebugString() << std::endl;
}
...
void testBlockClient()
{
    ...
    std::cout << "call query_name corpc server, req = " << nameReq.ShortDebugString() << std::endl;
    stub.query_name(&rpcControllerName, &nameReq, &nameRes, nullptr);
    std::cout << "peer addr: " << rpcControllerName.PeerAddr()->toString() << std::endl;
    ...
    std::cout << "Success get query_name response from corpc server, res = " << nameRes.ShortDebugString() << std::endl;
    ...
    std::cout << "call query_age corpc server, req = " << ageReq.ShortDebugString() << std::endl;
    stub.query_age(&rpcControllerAge, &ageReq, &ageRes, nullptr);
    std::cout << "peer addr: " << rpcControllerAge.PeerAddr()->toString() << std::endl;
...
void testNonBlockClient()
{
    ...
    corpc::PbRpcClientChannel::ptr channel = std::make_shared<corpc::PbRpcClientChannel>(addrs, corpc::LoadBalanceCategory::Random);
    ...
    auto cbAge = [ageRes, rpcControllerAge]() {
        ...
        if (ageRes->ret_code() != 0) {
            std::cout << "nonblock call query_age return bad result from corpc server, ret = " << ageRes->ret_code() << ", and res_info = " << ageRes->res_info();
            USER_LOG_DEBUG << "nonblock call query_age return bad result from corpc server, ret = " << ageRes->ret_code() << ", and res_info = " << ageRes->res_info();
            return;
        }
    ...
    std::cout << "nonblock call query_age corpc server, req = " << ageReq->ShortDebugString() << std::endl;
...
void testClientAsync()
{
    ...
    corpc::PbRpcClientAsyncChannel::ptr channel = std::make_shared<corpc::PbRpcClientAsyncChannel>(addrs, corpc::LoadBalanceCategory::ConsistentHash);
    QueryService_Stub stub(channel.get());
    ...
    auto cbAge = [ageRes, rpcControllerAge]() {
        ...
        if (ageRes->ret_code() != 0) {
            std::cout << "async call query_age return bad result from corpc server, ret = " << ageRes->ret_code() << ", and res_info = " << ageRes->res_info();
            USER_LOG_DEBUG << "async call query_age return bad result from corpc server, ret = " << ageRes->ret_code() << ", and res_info = " << ageRes->res_info();
            return;
        }
    ...
    std::cout << "async call query_age corpc server, req = " << ageReq->ShortDebugString() << std::endl;
    stub.query_age(rpcControllerAge.get(), ageReq.get(), ageRes.get(), closureAge.get());
